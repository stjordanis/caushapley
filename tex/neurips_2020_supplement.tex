...
\title{Causal Shapley values: explaining individual predictions when features are causally related}
...
\begin{abstract}
	...
	In this paper, we propose a novel framework for causal Shapley values that generalizes recent work that aims to relax or defend the independence assumption.
	% Our framework is based on an active, causal interpretation of feature attribution.
	Making use of Pearl's \textit{do}-calculus, we show how causal Shapley values can be derived for general causal graphs, without giving in on any of their desirable properties and with the possibility to separate direct from indirect effects. We provide a practical implementation for computing Shapley values based on causal chain graphs and illustrate causal Shapley values on several real-world examples.
\end{abstract}
...
Our active, interventional interpretation of Shapley values coincides with that in~\cite{janzing2019feature,lundberg2020local}.  
