...
\begin{description}
	...
	\item[Symmetry:] if $\val(S \cup i) = \val(S \cup j)$ for all  $S \subseteq \allfeatures \setminus \{i,j\}$, then $\contribution_i = \contribution_j$. \added[id=1]{Note that symmetry here is defined w.r.t.\ to the contributions $\contribution_i$, not the function values $f(\vx)$. As discussed in~\cite{janzing2019feature} (Section 3), conditioning by observation or intervention then does not break the symmetry property. So,} symmetry in this sense holds for marginal, conditional, and causal Shapley values\added[id=1]{ alike}.
\end{description}
