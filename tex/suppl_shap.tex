...
\]
where in the second step we implicitly made use of {\em do}-calculus rule~(1): the conditional independencies in the causal chain graph $G$ are preserved when we intervene on some of the variables, that is
\[
P(\vX_{\tau}|\vX_{\tau' \prec_G \tau},\svdo{S}) = P(\vX_{\tau}|\vX_{\spa(\tau)},\svdo{S}) \: .
\]
